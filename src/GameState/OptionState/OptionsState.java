...
public class OptionsState extends GameState{
	...
	public void saveConfig() throws IOException
	{
		...
		{
			int[] keys = new int[5];
			TextBlock textBlock = (TextBlock) blocks.get(a * 9 + 4);
			BoolBlock boolBlock = (BoolBlock) blocks.get(a * 9 + 5);
			TeamOptionBlock teamBlock = (TeamOptionBlock) blocks.get(a * 9 + 11);
			ImageBlock imageBlock = (ImageBlock) blocks.get(a * 9 + 12);
			...
			{
				KeyBlock block = (KeyBlock) blocks.get(a * 9 + 6 + b);
				keys[b] = block.getKey();
			}
		...
		BufferedWriter writer = new BufferedWriter(new FileWriter(new File(GamePanel.getConfigFilePath())));
	...
	private void initOptionBlocks()
	{
		...
		for(int a = 0; a < keysets.length; a++)
		{
			...
			name.setNearbyBlock(OptionBlock.UPPER_BLOCK, blocks.get(1));
		...
		blocks.get(1).setNearbyBlock(OptionBlock.BOTTOM_BLOCK, blocks.get(4));
		blocks.get(2).setNearbyBlock(OptionBlock.BOTTOM_BLOCK, blocks.get(22));
		blocks.get(22).setNearbyBlock(OptionBlock.UPPER_BLOCK, blocks.get(2));
		blocks.get(3).setNearbyBlock(OptionBlock.BOTTOM_BLOCK, blocks.get(31));
		blocks.get(31).setNearbyBlock(OptionBlock.UPPER_BLOCK, blocks.get(3));
	...
	private class FileSelectorBlock extends TextBlock
	{
		...
		public void keyPressed(int k)
		{
			...
			{
				fileChooser.setCurrentDirectory(new File(GamePanel.getRootFolderPath()));
